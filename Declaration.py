...
select ref_sinistre as ref_sinistre , dec.agence  as agence, dec.branche  as branche, trunc(date_declaration) as date_declaration  , dec.LIBELLE_TYPE_SINISTRE as  LIBELLE_TYPE_SINISTRE,
    ...
    and trunc(date_provision)  <= trunc(current_date)
    and      t.agence <> '00000'
    and t.reference = nb.reference 
    and t.region = nb.region 

   and       trunc(dec.date_declaration,'y')  = trunc(current_date,'y')
    and      trunc(dec.date_declaration)  <= trunc(current_date)
...
dec.to_csv(r'\\qnap\dcg\DCG_data warehouse\Declarations\declaration 2.csv', decimal=',')

try:
    os.remove(r'\\qnap\dcg\DCG_data warehouse\Declarations\declaration.csv')
except:
    pass

os.rename(r'\\qnap\dcg\DCG_data warehouse\Declarations\declaration 2.csv',
          r'\\qnap\dcg\DCG_data warehouse\Declarations\declaration.csv')
